...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zobsrObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zobsrObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zobsrObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zOBSRsupply", zobsrObj));
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in obsr\n"
...
UniValue getblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "  \"zOBSRsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zOBSR denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zOBSR denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zOBSR denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zOBSR denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zOBSR denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zOBSR denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zOBSR denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zOBSR denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zOBSR denominations\n"
            "  }\n"
            "}\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of obsr addresses\n"
            "     \"obsraddress\"   	 	(string) obsr address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, 100, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZOBSR_FAILED_ACCUMULATOR_INITIALIZATION);
