...
#if defined(NDEBUG)
#error "OBSR cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uobsr) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zOBSR spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zOBSR is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zOBSR spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zOBSR without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zOBSR spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zOBSR spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zobsr");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zOBSR mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        if (nHeight <= 101 && nHeight > 0)
            return 28125000 * COIN;
    }

    int64_t nSubsidy = 0;
    if (nHeight == 0) {
    /* POW1 - PHASE 1 - Initial Premine */
        nSubsidy = 8437500000 * COIN;
    } else if (nHeight == 1) {
        nSubsidy = 28125000 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZOBSRStake)
{
    int64_t ret = 0;
    ...
    if (nHeight <= 1001 && nHeight > Params().LAST_POW_BLOCK() && nHeight < Params().Zerocoin_Block_V2_Start()) {
        //return GetSeeSaw(blockValue, nMasternodeCount, nHeight);
        ret = blockValue / (100 / 75);
    } else if (nHeight <= 1001 && nHeight > Params().LAST_POW_BLOCK() && nHeight >= Params().Zerocoin_Block_V2_Start()) {
        ret = blockValue / (100 / 75);
    } else if (nHeight <= 20000 && nHeight > 1001) {
        ret = blockValue / (100 / 75);
    } else {
...
CBitcoinAddress addressExp1("oH3yHsSbE76oSH7UaG8dejrdd4bKuozbmF");
CBitcoinAddress addressExp2("oSvHfTzJm9MzB8fnKyyU3HK2TMiW6x46X9");
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from OBSR
...
{
    RenameThread("obsr-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZOBSRMinted()
{
...
void RecalculateZOBSRSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zOBSR supply
...
bool RecalculateOBSRSupply(int nHeightStart)
{
...
{
    // OBSR: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZOBSRSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateOBSRSupply(Params().Zerocoin_StartHeight());
    }

    //Track zOBSR money supply in the block index
    if (!UpdateZOBSRSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zOBSR supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zObsrSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert OBSR to zOBSR
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zOBSR spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zOBSR serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zOBSR stake block is less than allowed start height", __func__);

    if (CZObsrStake* zOBSR = dynamic_cast<CZObsrStake*>(stake)) {
        CBlockIndex* pindexFrom = zOBSR->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zOBSR stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zOBSR stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zOBSR->GetValue()));
        if (nChecksum200 != zOBSR->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zOBSR->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZOBSR() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zOBSR fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> obsrInputs;
        std::vector<CTxIn> zOBSRInputs;

        for (CTxIn stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zOBSRInputs.push_back(stakeIn);
            }else{
                obsrInputs.push_back(stakeIn);
            }
        }
        const bool hasOBSRInputs = !obsrInputs.empty();
        const bool hasZOBSRInputs = !zOBSRInputs.empty();
        ...
        for (CTransaction tx : block.vtx) {
            for (CTxIn in: tx.vin) {
                ...
                if(hasOBSRInputs)
                    ...
                    for (CTxIn obsrIn : obsrInputs){
                        if(obsrIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zOBSR inputs.
            if(hasZOBSRInputs){
                LogPrintf("It has ZOBSR Inputs!!\n");
                for (CTxIn zObsrInput : zOBSRInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zObsrInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zobsr");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zOBSR mints and %d zOBSR spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // OBSR: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zobsr", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zobsr", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
