...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZObsrStake(spend));
    } else {
        ...
        CObsrStake* obsrInput = new CObsrStake();
        obsrInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(obsrInput);
    }
